...
//            "ORDER BY r.time DESC", nativeQuery = true)
     ...
     */
    @Query(countQuery ="SELECT COUNT( DISTINCT r.id) FROM route r INNER JOIN car c ON r.car_id = c.id INNER JOIN position p ON r.id = p.route_id WHERE c.username = ?1", nativeQuery = true)
     ...
     */
    @Query(countQuery ="SELECT COUNT( DISTINCT r.id) FROM route r INNER JOIN position p ON r.id = p.route_id WHERE r.car_id = ?1", nativeQuery = true)
